...
package RP2350_SVD.POWMAN is
   ...
   --  that results in swcore being powered and xip unpowered If the request is
   ...
   type STATE_Register is record
      --  Read-only. Indicates the current power state
      ...
      --  Write data bit of one shall clear (set to zero) the corresponding bit
      --  in the field. Indicates that a software state change request was
      --  ignored because it clashed with an ongoing hardware or debugger
      --  request
      REQ_IGNORED         : Boolean := False;
      --  Write data bit of one shall clear (set to zero) the corresponding bit
      --  in the field. Indicates that a power state change request was ignored
      --  because of a pending power state change request
      PWRUP_WHILE_WAITING : Boolean := False;
      --  Read-only. Invalid software initiated state request ignored
      BAD_SW_REQ          : Boolean := False;
      --  Read-only. Invalid hardware initiated state request, power up
      --  requests actioned, power down requests ignored
      BAD_HW_REQ          : Boolean := False;
      --  Read-only. Indicates the power manager has received a state change
      --  request and is waiting for other actions to complete before executing
      --  it
      WAITING             : Boolean := False;
      --  Read-only. Indicates a power state change is in progress

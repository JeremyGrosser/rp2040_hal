...
package RP2350_SVD.I2C is
   ...
   --  This bit controls whether the DW_apb_i2c master is enabled. NOTE:
   ...
   --  This register should be programmed only with a value in the range of 1
   --  to IC_MAX_SPEED_MODE; otherwise, hardware updates this register with the
   --  value of IC_MAX_SPEED_MODE. 1: standard mode (100 kbit/s) 2: fast mode
   --  (<=400 kbit/s) or fast mode plus (<=1000Kbit/s) 3: high speed mode (3.4
   --  Mbit/s) Note: This field is not applicable when IC_ULTRA_FAST_MODE=1
   ...
   --  being set to 0. Writes at other times have no effect. Read/Write Access:
   --  - bit 10 is read only. - bit 11 is read only - bit 16 is read only - bit
   --  17 is read only - bits 18 and 19 are read only.
   type IC_CON_Register is record
      ...
      --  the speed mode. This register should be programmed only with a value
      --  in the range of 1 to IC_MAX_SPEED_MODE; otherwise, hardware updates
      --  this register with the value of IC_MAX_SPEED_MODE. 1: standard mode
      --  (100 kbit/s) 2: fast mode (<=400 kbit/s) or fast mode plus
      --  (<=1000Kbit/s) 3: high speed mode (3.4 Mbit/s) Note: This field is
      --  not applicable when IC_ULTRA_FAST_MODE=1
      SPEED                     : IC_CON_SPEED_Field := RP2350_SVD.I2C.FAST;
      --  When acting as a slave, this bit controls whether the DW_apb_i2c
      --  responds to 7- or 10-bit addresses. - 0: 7-bit addressing. The
      --  DW_apb_i2c ignores transactions that involve 10-bit addressing; for
      --  7-bit addressing, only the lower 7 bits of the IC_SAR register are
      ...
      --  of the IC_RAW_INTR_STAT register. Reset value: ENABLED
      ...
      --  slave is disabled. If this bit is set (slave is disabled), DW_apb_i2c
      --  functions only as a master and does not perform any action that
      --  requires a slave. NOTE: Software should ensure that if this bit is
      --  written with 0, then bit 0 should also be written with a 0.
      IC_SLAVE_DISABLE          : IC_CON_IC_SLAVE_DISABLE_Field :=
                                   RP2350_SVD.I2C.SLAVE_DISABLED;
      --  In slave mode: - 1'b1: issues the STOP_DET interrupt only when it is
      --  addressed. - 1'b0: issues the STOP_DET irrespective of whether it's
      --  addressed or not. Reset value: 0x0 NOTE: During a general call
      ...
      STOP_DET_IFADDRESSED      : IC_CON_STOP_DET_IFADDRESSED_Field :=
                                   RP2350_SVD.I2C.DISABLED;
      --  This bit controls the generation of the TX_EMPTY interrupt, as
      --  described in the IC_RAW_INTR_STAT register. Reset value: 0x0.
   ...
   --  I2C Target Address Register This register is 12 bits wide, and bits
   --  31:12 are reserved. This register can be written to only when
   --  IC_ENABLE[0] is set to 0. Note: If the software or application is aware
   --  that the DW_apb_i2c is not using the TAR address for the pending
   ...
   type IC_TAR_Register is record
      ...
      --  START BYTE, the CPU needs to write only once into these bits. If the
      --  IC_TAR and IC_SAR are the same, loopback exists but the FIFOs are
      ...
      SPECIAL        : IC_TAR_SPECIAL_Field := RP2350_SVD.I2C.DISABLED;
      --  unspecified
   ...
   type IC_SAR_Register is record
      ...
      --  slave. For 7-bit addressing, only IC_SAR[6:0] is used. This register
      ...
      --  other times have no effect. Note: The default values cannot be any of
      --  the reserved address locations: that is, 0x00 to 0x07, or 0x78 to
      --  0x7f. The correct operation of the device is not guaranteed if you
   ...
   --  received. - 1 - STOP is issued after this byte, regardless of whether or
   --  not the Tx FIFO is empty. If the Tx FIFO is not empty, the master
   ...
   --  : 0x0 NOTE: In case of APB_DATA_WIDTH=8, 1. The user has to perform two
   --  APB Reads to IC_DATA_CMD in order to get status on 11 bit. 2. In order
   --  to read the 11 bit, the user has to perform the first data byte read
   --  [7:0] (offset 0x10) and then perform the second read [15:8] (offset
   --  0x11) in order to know the status of 11 bit (whether the data received
   --  in previous read is a first data byte or not). 3. The 11th bit is an
   --  optional read field, user can ignore 2nd byte read [15:8] (offset 0x11)
   ...
   --  retrieving bytes from RX FIFO. The size of the register changes as
   --  follows: Write: - 11 bits when IC_EMPTYFIFO_HOLD_MASTER_EN=1 - 9 bits
   ...
   type IC_DATA_CMD_Register is record
      ...
      --  the DW_apb_i2c interface. Reset value: 0x0
      DAT             : IC_DATA_CMD_DAT_Field := 16#0#;
      --  Write-only. This bit controls whether a read or a write is performed.
      ...
      --  is to be transmitted. When programming this bit, you should remember
      --  the following: attempting to perform a read operation after a General
      --  Call command has been sent results in a TX_ABRT interrupt (bit 6 of
      --  the IC_RAW_INTR_STAT register), unless bit 11 (SPECIAL) in the IC_TAR
      --  register has been cleared. If a '1' is written to this bit after
      --  receiving a RD_REQ interrupt, then a TX_ABRT interrupt occurs. Reset
      --  value: 0x0
      CMD             : IC_DATA_CMD_CMD_Field := RP2350_SVD.I2C.WRITE;
      --  Write-only. This bit controls whether a STOP is issued after the byte
      --  is sent or received. - 1 - STOP is issued after this byte, regardless
      --  of whether or not the Tx FIFO is empty. If the Tx FIFO is not empty,
      --  the master immediately tries to start a new transfer by issuing a
      --  START and arbitrating for the bus. - 0 - STOP is not issued after
      ...
      --  0x0
      STOP            : IC_DATA_CMD_STOP_Field := RP2350_SVD.I2C.DISABLE;
      --  Write-only. This bit controls whether a RESTART is issued before the
      --  byte is sent or received. 1 - If IC_RESTART_EN is 1, a RESTART is
      ...
      --  a START is issued instead. 0 - If IC_RESTART_EN is 1, a RESTART is
      ...
      RESTART         : IC_DATA_CMD_RESTART_Field := RP2350_SVD.I2C.DISABLE;
      ...
      --  Reset value : 0x0 NOTE: In case of APB_DATA_WIDTH=8, 1. The user has
      --  to perform two APB Reads to IC_DATA_CMD in order to get status on 11
      --  bit. 2. In order to read the 11 bit, the user has to perform the
      --  first data byte read [7:0] (offset 0x10) and then perform the second
      --  read [15:8] (offset 0x11) in order to know the status of 11 bit
      --  (whether the data received in previous read is a first data byte or
      --  not). 3. The 11th bit is an optional read field, user can ignore 2nd
      --  byte read [15:8] (offset 0x11) if not interested in FIRST_DATA_BYTE
      --  status.
      FIRST_DATA_BYTE : IC_DATA_CMD_FIRST_DATA_BYTE_Field :=
                         RP2350_SVD.I2C.INACTIVE;
      --  unspecified
      Reserved_12_31  : HAL.UInt20 := 16#0#;
   end record
     with Volatile_Full_Access, Object_Size => 32,
   ...
   type IC_SS_SCL_HCNT_Register is record
      ...
      --  'IC_CLK Frequency Configuration'. This register can be written only
      ...
      --  designs with APB_DATA_WIDTH = 8, the order of programming is
      ...
      --  programmed. NOTE: This register must not be programmed to a value
   ...
   type IC_SS_SCL_LCNT_Register is record
      ...
      --  when the I2C interface is disabled which corresponds to the
      --  IC_ENABLE[0] register being set to 0. Writes at other times have no
      --  effect. The minimum valid value is 8; hardware prevents values less
      --  than this being written, and if attempted, results in 8 being set.
      --  For designs with APB_DATA_WIDTH = 8, the order of programming is
   ...
   type IC_FS_SCL_HCNT_Register is record
      ...
      --  Configuration'. This register goes away and becomes read-only
   ...
   type IC_FS_SCL_LCNT_Register is record
      ...
      --  information, refer to 'IC_CLK Frequency Configuration'. This register
      ...
      --  disabled, which corresponds to the IC_ENABLE[0] register being set to
      --  0. Writes at other times have no effect. The minimum valid value is
      --  8; hardware prevents values less than this being written, and if
      --  attempted results in 8 being set. For designs with APB_DATA_WIDTH = 8
      --  the order of programming is important to ensure the correct operation
      --  of the DW_apb_i2c. The lower byte must be programmed first. Then the
      --  upper byte is programmed. If the value is less than 8 then the count
   ...
   --  See IC_RAW_INTR_STAT for a detailed description of R_RX_UNDER bit. Reset
   --  value: 0x0
   ...
   --  See IC_RAW_INTR_STAT for a detailed description of R_RX_OVER bit. Reset
   --  value: 0x0
   ...
   --  See IC_RAW_INTR_STAT for a detailed description of R_RX_FULL bit. Reset
   --  value: 0x0
   ...
   --  See IC_RAW_INTR_STAT for a detailed description of R_TX_OVER bit. Reset
   --  value: 0x0
   ...
   --  See IC_RAW_INTR_STAT for a detailed description of R_TX_EMPTY bit. Reset
   --  value: 0x0
   ...
   --  See IC_RAW_INTR_STAT for a detailed description of R_RD_REQ bit. Reset
   --  value: 0x0
   ...
   --  See IC_RAW_INTR_STAT for a detailed description of R_TX_ABRT bit. Reset
   --  value: 0x0
   ...
   --  See IC_RAW_INTR_STAT for a detailed description of R_RX_DONE bit. Reset
   --  value: 0x0
   ...
   --  See IC_RAW_INTR_STAT for a detailed description of R_ACTIVITY bit. Reset
   --  value: 0x0
   ...
   --  See IC_RAW_INTR_STAT for a detailed description of R_STOP_DET bit. Reset
   --  value: 0x0
   ...
   --  See IC_RAW_INTR_STAT for a detailed description of R_GEN_CALL bit. Reset
   --  value: 0x0
   ...
   --  This bit masks the R_RX_UNDER interrupt in IC_INTR_STAT register. Reset
   --  value: 0x1
   ...
   --  This bit masks the R_RX_OVER interrupt in IC_INTR_STAT register. Reset
   --  value: 0x1
   ...
   --  This bit masks the R_RX_FULL interrupt in IC_INTR_STAT register. Reset
   --  value: 0x1
   ...
   --  This bit masks the R_TX_OVER interrupt in IC_INTR_STAT register. Reset
   --  value: 0x1
   ...
   --  This bit masks the R_TX_EMPTY interrupt in IC_INTR_STAT register. Reset
   --  value: 0x1
   ...
   --  This bit masks the R_RD_REQ interrupt in IC_INTR_STAT register. Reset
   --  value: 0x1
   ...
   --  This bit masks the R_TX_ABRT interrupt in IC_INTR_STAT register. Reset
   --  value: 0x1
   ...
   --  This bit masks the R_RX_DONE interrupt in IC_INTR_STAT register. Reset
   --  value: 0x1
   ...
   --  This bit masks the R_ACTIVITY interrupt in IC_INTR_STAT register. Reset
   --  value: 0x0
   ...
   --  This bit masks the R_STOP_DET interrupt in IC_INTR_STAT register. Reset
   --  value: 0x0
   ...
   --  This bit masks the R_START_DET interrupt in IC_INTR_STAT register. Reset
   --  value: 0x0
   ...
   --  This bit masks the R_GEN_CALL interrupt in IC_INTR_STAT register. Reset
   --  value: 0x1
   ...
   --  I2C Interrupt Mask Register. These bits mask their corresponding
   ...
   --  interrupt will be issued only if slave is addressed. Note: During a
   --  general call address, this slave does not issue a STOP_DET interrupt if
   --  STOP_DET_IF_ADDRESSED=1'b1, even if the slave responds to the general
   --  call address by generating ACK. The STOP_DET interrupt is generated only
   --  when the transmitted address matches the slave address (SAR). - If
   --  IC_CON[7]=1'b0 (STOP_DET_IFADDRESSED), the STOP_DET interrupt is issued
   --  irrespective of whether it is being addressed. In Master Mode: - If
   --  IC_CON[10]=1'b1 (STOP_DET_IF_MASTER_ACTIVE),the STOP_DET interrupt will
   ...
   --  I2C Raw Interrupt Status Register Unlike the IC_INTR_STAT register,
   ...
   type IC_RAW_INTR_STAT_Register is record
      ...
      --  interrupt is cleared. Note: If bit 9 of the IC_CON register
      --  (RX_FIFO_FULL_HLD_CTRL) is programmed to HIGH, then the RX_OVER
      --  interrupt never occurs, because the Rx FIFO never overflows. Reset
      --  value: 0x0
      ...
      --  activity that continues. Reset value: 0x0
      ...
      --  ic_en=0, this bit is set to 0. Reset value: 0x0.
      ...
      --  processor reads the IC_CLR_RD_REQ register. Reset value: 0x0
      ...
      --  transmit abort takes places. Note: The DW_apb_i2c
      ...
      --  the APB interface. Reset value: 0x0
      ...
      --  that the transmission is done. Reset value: 0x0
      ...
      --  register. DW_apb_i2c stores the received data in the Rx buffer. Reset
      --  value: 0x0
      ...
      --  slave is being addressed. Enabled only when IC_SLV_RESTART_DET_EN=1.
      --  Note: However, in high-speed mode or during a START BYTE transfer,
      --  the RESTART comes before the address field as per the I2C protocol.
      --  In this case, the slave is not the addressed slave when the RESTART
      --  is issued, therefore DW_apb_i2c does not generate the RESTART_DET
      --  interrupt. Reset value: 0x0
   ...
   type IC_TX_TL_Register is record
      --  Transmit FIFO Threshold Level. Controls the level of entries (or
   ...
   type IC_CLR_TX_ABRT_Register is record
      ...
      --  register for an exception to clearing IC_TX_ABRT_SOURCE. Reset value:
      --  0x0
      CLR_TX_ABRT   : Boolean;
      --  unspecified
      Reserved_1_31 : HAL.UInt31;
   ...
   type IC_CLR_START_DET_Register is record
      ...
      --  10) of the IC_RAW_INTR_STAT register. Reset value: 0x0
   ...
   type IC_CLR_GEN_CALL_Register is record
      ...
      --  11) of IC_RAW_INTR_STAT register. Reset value: 0x0
   ...
   type IC_ENABLE_Register is record
      ...
      --  important that care be taken to ensure that DW_apb_i2c is disabled
      --  properly. A recommended procedure is described in 'Disabling
      --  DW_apb_i2c'. When DW_apb_i2c is disabled, the following occurs: - The
      ...
      --  end of the current byte and does not acknowledge the transfer. In
      --  systems with asynchronous pclk and ic_clk when IC_CLK_TYPE parameter
      --  set to asynchronous (1), there is a two ic_clk delay when enabling or
      --  disabling the DW_apb_i2c. For a detailed description on how to
      --  disable DW_apb_i2c, refer to 'Disabling DW_apb_i2c' Reset value: 0x0
      ENABLE        : IC_ENABLE_ENABLE_Field := RP2350_SVD.I2C.DISABLED;
      --  When set, the controller initiates the transfer abort. - 0: ABORT not
      --  initiated or ABORT done - 1: ABORT operation in progress The software
      --  can abort the I2C transfer in master mode by setting this bit. The
      --  software can set this bit only when ENABLE is already set; otherwise,
      ...
      --  The ABORT bit is cleared automatically after the abort operation. For
      --  a detailed description on how to abort I2C transfers, refer to
      --  'Aborting I2C Transfers'. Reset value: 0x0
      ABORT_k       : IC_ENABLE_ABORT_Field := RP2350_SVD.I2C.DISABLE;
   ...
   --  register: - Bits 1 and 2 are set to 1 - Bits 3 and 10 are set to 0 When
   --  the master or slave state machines goes to idle and ic_en=0: - Bits 5
   --  and 6 are set to 0
   type IC_STATUS_Register is record
      ...
      --  SLV_ACTIVITY and MST_ACTIVITY bits. Reset value: 0x0
   ...
   type IC_TXFLR_Register is record
      ...
      --  entries in the transmit FIFO. Reset value: 0x0
   ...
   type IC_RXFLR_Register is record
      ...
      --  entries in the receive FIFO. Reset value: 0x0
   ...
   --  register succeed only when IC_ENABLE[0]=0. The values in this register
   --  are in units of ic_clk period. The value programmed in IC_SDA_TX_HOLD
   --  must be greater than the minimum hold time in each mode (one cycle in
   --  master mode, seven cycles in slave mode) for the value to be
   --  implemented. The programmed SDA hold time during transmit
   ...
   type IC_SDA_HOLD_Register is record
      ...
      --  Sets the required SDA hold time in units of ic_clk period, when
      --  DW_apb_i2c acts as a receiver. Reset value:
...
--  present
   ...
   --  I2C Transmit Abort Source Register This register has 32 bits that
   ...
   type IC_TX_ABRT_SOURCE_Register is record
      ...
      --  slave. Reset value: 0x0 Role of DW_apb_i2c: Master-Transmitter or
      --  Master-Receiver
      ...
      --  acknowledged by any slave. Reset value: 0x0 Role of DW_apb_i2c:
      --  Master-Transmitter or Master-Receiver
      ...
      --  from the remote slave(s). Reset value: 0x0 Role of DW_apb_i2c:
      --  Master-Transmitter
      ...
      --  Call. Reset value: 0x0 Role of DW_apb_i2c: Master-Transmitter
      ...
      --  1). Reset value: 0x0 Role of DW_apb_i2c: Master-Transmitter
      ...
      --  and the START Byte was acknowledged (wrong behavior). Reset value:
      --  0x0 Role of DW_apb_i2c: Master
      ...
      --  master to transfer data in High Speed mode. Reset value: 0x0 Role of
      --  DW_apb_i2c: Master-Transmitter or Master-Receiver
      ...
      --  Master operation with the Master mode disabled. Reset value: 0x0 Role
      ...
      --  TX_ABRT interrupt to flush old data in TX FIFO. Reset value: 0x0 Role
      ...
      --  transmitted, then DW_apb_i2c no longer own the bus. Reset value: 0x0
      ...
      --  a 1 in CMD (bit 8) of IC_DATA_CMD register. Reset value: 0x0 Role of
      --  DW_apb_i2c: Slave-Transmitter
      ABRT_SLVRD_INTX      : IC_TX_ABRT_SOURCE_ABRT_SLVRD_INTX_Field;
      --  Read-only. This is a master-mode-only bit. Master has detected the
      --  transfer abort (IC_ENABLE[1]) Reset value: 0x0 Role of DW_apb_i2c:
      --  Master-Transmitter
      ...
      --  I2C is disabled. Reset value: 0x0 Role of DW_apb_i2c:
      --  Master-Transmitter or Slave-Transmitter
   ...
   --  depending on normal criteria. - 1: generate NACK after data byte
   --  received - 0: generate NACK/ACK normally Reset value: 0x0
   ...
   --  Generate Slave Data NACK Register The register is used to generate a
   ...
   --  not exist and writing to the register's address has no effect. A write
   --  can occur on this register if both of the following conditions are met:
   --  - DW_apb_i2c is disabled (IC_ENABLE[0] = 0) - Slave part is inactive
   --  (IC_STATUS[6] = 0) Note: The IC_STATUS[6] is a register read-back
   --  location for the internal slv_activity signal; the user should poll this
   ...
   type IC_SLV_DATA_NACK_ONLY_Register is record
      ...
      --  receive buffer. When the register is set to a value of 0, it
   ...
   --  DMA Control Register The register is used to enable the DMA Controller
   ...
   type IC_DMA_CR_Register is record
      --  Receive DMA Enable. This bit enables/disables the receive FIFO DMA
      --  channel. Reset value: 0x0
      RDMAE         : IC_DMA_CR_RDMAE_Field := RP2350_SVD.I2C.DISABLED;
      --  Transmit DMA Enable. This bit enables/disables the transmit FIFO DMA
      --  channel. Reset value: 0x0
      TDMAE         : IC_DMA_CR_TDMAE_Field := RP2350_SVD.I2C.DISABLED;
      --  unspecified
   ...
   type IC_DMA_TDLR_Register is record
      ...
      --  field value, and TDMAE = 1. Reset value: 0x0
   ...
   --  I2C SDA Setup Register This register controls the amount of time delay
   --  (in terms of number of ic_clk clock periods) introduced in the rising
   --  edge of SCL - relative to SDA changing - when DW_apb_i2c services a read
   --  request in a slave-transmitter operation. The relevant I2C requirement
   --  is tSU:DAT (note 4) as detailed in the I2C Bus Specification. This
   --  register must be programmed with a value equal to or greater than 2.
   --  Writes to this register succeed only when IC_ENABLE[0] = 0. Note: The
   --  length of setup time is calculated using [(IC_SDA_SETUP - 1) *
   --  (ic_clk_period)], so if the user requires 10 ic_clk periods of setup
   --  time, they should program a value of 11. The IC_SDA_SETUP register is
   --  only used by the DW_apb_i2c when operating as a slave transmitter.
   ...
   --  responds with a ACK or NACK when it receives an I2C General Call
   --  address. This register is applicable only when the DW_apb_i2c is in
   --  slave mode.
   ...
   --  is, when DW_apb_i2c is disabled. If IC_ENABLE[0] has been set to 1, bits
   --  2:1 are forced to 0, and bit 0 is forced to 1. If IC_ENABLE[0] has been
   --  set to 0, bits 2:1 is only be valid as soon as bit 0 is read as '0'.
   --  Note: When IC_ENABLE[0] has been set to 0, a delay occurs for bit 0 to
   --  be read as 0 because disabling the DW_apb_i2c depends on I2C bus
   --  activities.
   type IC_ENABLE_STATUS_Register is record
      ...
      --  (bit 2) and SLV_DISABLED_WHILE_BUSY (bit 1). Reset value: 0x0
      ...
      --  operation from a remote master; OR, (b) address and data bytes of the
      --  Slave-Receiver operation from a remote master. When read as 1,
      ...
      --  When read as 0, DW_apb_i2c is deemed to have been disabled when there
      --  is master activity, or when the I2C bus is idle. Note: The CPU can
      --  safely read this bit when IC_EN (bit 0) is read as 0. Reset value:
      --  0x0
      ...
      --  a data byte has been responded with a NACK. Note: If the remote I2C
      --  master terminates the transfer with a STOP condition before the
      --  DW_apb_i2c has a chance to NACK a transfer, and IC_ENABLE[0] has been
      --  set to 0, then this bit is also set to 1. When read as 0, DW_apb_i2c
      --  is deemed to have been disabled without being actively involved in
      --  the data phase of a Slave-Receiver transfer. Note: The CPU can safely
      --  read this bit when IC_EN (bit 0) is read as 0. Reset value: 0x0
   ...
   --  filtered out by the spike suppression logic when the component is
   ...
   type IC_CLR_RESTART_DET_Register is record
      ...
      --  12) of IC_RAW_INTR_STAT register. Reset value: 0x0
   ...
   --  Component Parameter Register 1 Note This register is not implemented and
   --  therefore reads as 0. If it was implemented it would be a constant
   ...
   --  DW_apb_i2c address block List of configuration constants for the
   ...
   --  IC_ULTRA_FAST_MODE ................ 0x0 IC_UFM_TBUF_CNT_DEFAULT
   --  ........... 0x8 IC_UFM_SCL_LOW_COUNT .............. 0x0008
   --  IC_UFM_SCL_HIGH_COUNT ............. 0x0006 IC_TX_TL
   --  .......................... 0x0 IC_TX_CMD_BLOCK ................... 0x1
   --  IC_HAS_DMA ........................ 0x1 IC_HAS_ASYNC_FIFO
   --  ................. 0x0 IC_SMBUS_ARP ...................... 0x0
   --  IC_FIRST_DATA_BYTE_STATUS ......... 0x1 IC_INTR_IO
   --  ........................ 0x1 IC_MASTER_MODE .................... 0x1
   --  IC_DEFAULT_ACK_GENERAL_CALL ....... 0x1 IC_INTR_POL
   --  ....................... 0x1 IC_OPTIONAL_SAR ................... 0x0
   --  IC_DEFAULT_TAR_SLAVE_ADDR ......... 0x055 IC_DEFAULT_SLAVE_ADDR
   --  ............. 0x055 IC_DEFAULT_HS_SPKLEN .............. 0x1
   --  IC_FS_SCL_HIGH_COUNT .............. 0x0006 IC_HS_SCL_LOW_COUNT
   --  ............... 0x0008 IC_DEVICE_ID_VALUE ................ 0x0
   --  IC_10BITADDR_MASTER ............... 0x0 IC_CLK_FREQ_OPTIMIZATION
   --  .......... 0x0 IC_DEFAULT_FS_SPKLEN .............. 0x7
   --  IC_ADD_ENCODED_PARAMS ............. 0x0 IC_DEFAULT_SDA_HOLD
   --  ............... 0x000001 IC_DEFAULT_SDA_SETUP .............. 0x64
   --  IC_AVOID_RX_FIFO_FLUSH_ON_TX_ABRT . 0x0 IC_CLOCK_PERIOD
   --  ................... 100 IC_EMPTYFIFO_HOLD_MASTER_EN ....... 1
   --  IC_RESTART_EN ..................... 0x1 IC_TX_CMD_BLOCK_DEFAULT
   --  ........... 0x0 IC_BUS_CLEAR_FEATURE .............. 0x0 IC_CAP_LOADING
   --  .................... 100 IC_FS_SCL_LOW_COUNT ............... 0x000d
   --  APB_DATA_WIDTH .................... 32 IC_SDA_STUCK_TIMEOUT_DEFAULT
   --  ...... 0xffffffff IC_SLV_DATA_NACK_ONLY ............. 0x1
   --  IC_10BITADDR_SLAVE ................ 0x0 IC_CLK_TYPE
   --  ....................... 0x0 IC_SMBUS_UDID_MSB ................. 0x0
   --  IC_SMBUS_SUSPEND_ALERT ............ 0x0 IC_HS_SCL_HIGH_COUNT
   --  .............. 0x0006 IC_SLV_RESTART_DET_EN ............. 0x1 IC_SMBUS
   --  .......................... 0x0 IC_OPTIONAL_SAR_DEFAULT ........... 0x0
   --  IC_PERSISTANT_SLV_ADDR_DEFAULT .... 0x0 IC_USE_COUNTS
   --  ..................... 0x0 IC_RX_BUFFER_DEPTH ................ 16
   --  IC_SCL_STUCK_TIMEOUT_DEFAULT ...... 0xffffffff IC_RX_FULL_HLD_BUS_EN
   --  ............. 0x1 IC_SLAVE_DISABLE .................. 0x1 IC_RX_TL
   --  .......................... 0x0 IC_DEVICE_ID ...................... 0x0
   --  IC_HC_COUNT_VALUES ................ 0x0 I2C_DYNAMIC_TAR_UPDATE
   --  ............ 0 IC_SMBUS_CLK_LOW_MEXT_DEFAULT ..... 0xffffffff
   --  IC_SMBUS_CLK_LOW_SEXT_DEFAULT ..... 0xffffffff IC_HS_MASTER_CODE
   --  ................. 0x1 IC_SMBUS_RST_IDLE_CNT_DEFAULT ..... 0xffff
   --  IC_SMBUS_UDID_LSB_DEFAULT ......... 0xffffffff IC_SS_SCL_HIGH_COUNT
   --  .............. 0x0028 IC_SS_SCL_LOW_COUNT ............... 0x002f
   --  IC_MAX_SPEED_MODE ................. 0x2 IC_STAT_FOR_CLK_STRETCH
   --  ........... 0x0 IC_STOP_DET_IF_MASTER_ACTIVE ...... 0x0
   --  IC_DEFAULT_UFM_SPKLEN ............. 0x1 IC_TX_BUFFER_DEPTH
   --  ................ 16
   type I2C_Peripheral is record
      ...
      --  I2C Target Address Register This register is 12 bits wide, and bits
      --  31:12 are reserved. This register can be written to only when
      --  IC_ENABLE[0] is set to 0. Note: If the software or application is
      ...
      --  I2C Status Register This is a read-only register used to indicate the
      --  current transfer status and FIFO status. The status register may be
      --  read at any time. None of the bits in this register request an
      --  interrupt. When the I2C is disabled by writing 0 in bit 0 of the
      ...
      --  I2C SDA Hold Time Length Register The bits [15:0] of this register
      --  are used to control the hold time of SDA during transmit in both
      --  slave and master mode (after SCL goes from HIGH to LOW). The bits
      --  [23:16] of this register are used to extend the SDA transition (if
      --  any) whenever SCL is HIGH in the receiver in either master or slave
      --  mode. Writes to this register succeed only when IC_ENABLE[0]=0. The
      --  values in this register are in units of ic_clk period. The value
      --  programmed in IC_SDA_TX_HOLD must be greater than the minimum hold
      --  time in each mode (one cycle in master mode, seven cycles in slave
      --  mode) for the value to be implemented. The programmed SDA hold time
      --  during transmit (IC_SDA_TX_HOLD) cannot exceed at any time the
      --  duration of the low part of scl. Therefore the programmed value
      ...
      --  Generate Slave Data NACK Register The register is used to generate a
      --  NACK for the data part of a transfer when DW_apb_i2c is acting as a
      --  slave-receiver. This register only exists when the
      ...
      --  address has no effect. A write can occur on this register if both of
      --  the following conditions are met: - DW_apb_i2c is disabled
      ...
      --  DMA Control Register The register is used to enable the DMA
      ...
      --  I2C SDA Setup Register This register controls the amount of time
      ...
      --  equal to or greater than 2. Writes to this register succeed only when
      --  IC_ENABLE[0] = 0. Note: The length of setup time is calculated using
      ...
      --  IC_SDA_SETUP register is only used by the DW_apb_i2c when operating
      --  as a slave transmitter.
      IC_SDA_SETUP          : aliased IC_SDA_SETUP_Register;
      --  I2C ACK General Call Register The register controls whether
      --  DW_apb_i2c responds with a ACK or NACK when it receives an I2C
      --  General Call address. This register is applicable only when the
      --  DW_apb_i2c is in slave mode.
      IC_ACK_GENERAL_CALL   : aliased IC_ACK_GENERAL_CALL_Register;
      --  I2C Enable Status Register The register is used to report the
      --  DW_apb_i2c hardware status when the IC_ENABLE[0] register is set from
      --  1 to 0; that is, when DW_apb_i2c is disabled. If IC_ENABLE[0] has
      --  been set to 1, bits 2:1 are forced to 0, and bit 0 is forced to 1. If
      --  IC_ENABLE[0] has been set to 0, bits 2:1 is only be valid as soon as
      --  bit 0 is read as '0'. Note: When IC_ENABLE[0] has been set to 0, a
      --  delay occurs for bit 0 to be read as 0 because disabling the
      --  DW_apb_i2c depends on I2C bus activities.
      IC_ENABLE_STATUS      : aliased IC_ENABLE_STATUS_Register;
      --  I2C SS, FS or FM+ spike suppression limit This register is used to
      --  store the duration, measured in ic_clk cycles, of the longest spike
      --  that is filtered out by the spike suppression logic when the
      ...
      --  Component Parameter Register 1 Note This register is not implemented
      --  and therefore reads as 0. If it was implemented it would be a
      --  constant read-only register that contains encoded information about
      --  the component's parameter settings. Fields shown below are the
